...
class TransferCreateDTO(BaseModel):
    ...
    source_wallet_id: int = Field(
        ..., description="ID of the source wallet (money comes from)"
    )
    target_wallet_id: int = Field(
        ..., description="ID of the target wallet (money goes to)"
    )
    description: Optional[str] = Field(
        None, max_length=500, description="Optional description/note about the transfer"
    )
...
class TransferFilterDTO(BaseModel):
    ...
    source_wallet_id: Optional[int] = Field(
        None, description="Filter by source wallet ID"
    )
    target_wallet_id: Optional[int] = Field(
        None, description="Filter by target wallet ID"
    )
    wallet_id: Optional[int] = Field(
        None, description="Filter by transfers involving this wallet (source or target)"
    )
