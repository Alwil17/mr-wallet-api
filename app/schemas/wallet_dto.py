...
class WalletCreateDTO(BaseModel):
    ...
    type: str = Field(
        ...
        min_length=1,
        max_length=50,
        description="Wallet type (e.g., checking, savings, cash, credit)",
    )
    ...
    def validate_balance(self):
        ...
        if wallet_type != "credit" and balance is not None and balance < 0:
            raise ValueError(
                "Initial balance cannot be negative for non-credit wallets."
            )
        return self
...
class WalletUpdateDTO(BaseModel):
    ...
    name: Optional[str] = Field(
        None, min_length=1, max_length=100, description="Wallet name"
    )
    type: Optional[str] = Field(
        None, min_length=1, max_length=50, description="Wallet type"
    )
...
class WalletBalanceUpdateDTO(BaseModel):
    ...
    operation: str = Field(
        ...,
        pattern="^(add|subtract|set)$",
        description="Operation type: add, subtract, or set",
    )
    note: Optional[str] = Field(
        None, max_length=255, description="Note for the balance update"
    )
