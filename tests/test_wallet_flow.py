...
def test_wallet_management():
    ...
    try:
        ...
        if response.status_code == 400:
            ...
            print("⚠️  Overdraft protection may not be working properly")
        ...
        if response.status_code == 204:
            print("✅ Deleted cash wallet successfully")
            
            # Verify it's gone
            response = requests.get(f"{BASE_URL}/wallets/{cash_wallet['id']}", headers=headers)
            if response.status_code == 404:
                print("✅ Wallet properly deleted - not found")
            else:
                print("⚠️  Wallet may not have been properly deleted")
        else:
